...
## Patterns the workspace has standardized

- **Hashing is async, always.** Use `crypto.subtle.digest("SHA-256", ...)` — never `node:crypto`. Hashing helpers return `Promise<string>` so they work in CF Workers and Bun without forks.
- **`FetchLike` over `typeof fetch`.** Adapters that accept an injected fetch use `type FetchLike = (url: string, init: RequestInit) => Promise<Response>`. `typeof fetch` pulls in Bun's `preconnect` and forces test fakes to stub it.
- **Async only when awaiting.** `useAwait` is a Biome error. A function that returns `Promise.all(...)` or `Promise.resolve(...)` is a plain function, not `async`.
- **`process.env` access:** TS `noPropertyAccessFromIndexSignature` and Biome `useLiteralKeys` conflict on env access. Use bracket access `process.env["NAME"]` and add `// biome-ignore lint/complexity/useLiteralKeys: env access requires brackets under noPropertyAccessFromIndexSignature` on that line.
- **Convex `_generated/` policy.** `convex/_generated/` is committed to the repo, hand-maintainable, and re-generated by `convex codegen` when a deployment exists. `tests/helpers/client.ts` keeps a parallel modules map for `convex-test`; both must list every module under `packages/convex/convex/`.
- **MCP Worker → Convex trust boundary.** Public Convex queries/mutations call `requireUserId(ctx)`. The MCP Worker passes `X-OpenBrains-User-Id` + `X-OpenBrains-Internal-Secret` to `convex/http.ts`, which validates the secret then dispatches to **internal** mutations that accept `userId` as a parameter. Never expose those internal mutations directly — they bypass `requireUserId`.
- **TanStack Start file conventions.** Routes use default exports and TanStack-specific filenames (`__root.tsx`, `sign-in.$.tsx`) that violate `useFilenamingConvention` / `noDefaultExport`. The per-app override lives in root `biome.json`. shadcn primitives in `components/ui/**` are copy-paste and also need a relaxed override (default export + `useKeyWithClickEvents`). Don't add more route-style exceptions without justification.
- **Clerk v6 component shape.** No `<SignedIn>`/`<SignedOut>` — use `<Show when="signed-in">` / `<Show when="signed-out">` from `@clerk/tanstack-react-start`.
- **`import.meta.env` access (Vite).** Read via a typed accessor: `(import.meta as unknown as ImportMetaWithEnv).env.VITE_CONVEX_URL`. The `unknown` step is the boundary narrowing CLAUDE.md §2 requires — don't widen to `any`.
- **Convex workspace package exports.** `packages/convex/package.json` exports `./api` and `./dataModel` from `convex/_generated/`. Consumers `import { api } from "@openbrains/convex/api"`. Keep the rest of the generated surface internal.
- **`useLiteralKeys` vs `noPropertyAccessFromIndexSignature` — general rule.** This conflict appears any time a `Record<string, V>` is read. Pattern: write a typed accessor (`flagString(flags, name)`, `getEnv(env, name)`) and confine the bracket access **plus** the `// biome-ignore lint/complexity/useLiteralKeys` comment to that one accessor. Don't sprinkle the suppression at every call site.
- **`biome-ignore-all` placement.** The `biome-ignore-all` directive must be the literal first line of the file (above all imports). If it sits below an import, Biome silently ignores it.
- **MCP SDK tool output shape.** `ToolTextResult.structuredContent` is typed as `Record<string, unknown>` (not `unknown`) by `@modelcontextprotocol/sdk` 1.29+. Tools that return structured output must wrap in `{ ... }`, never a bare value.
- **`agents/mcp` is Worker-only at import time.** It transitively imports `cloudflare:email`, which Bun's test runtime can't resolve. Unit-test tool handlers by calling them directly with fakes; reserve `createMcpHandler` integration tests for Miniflare/vitest-pool-workers.
