import { memoryWriteback, ServiceAuthError, ServiceInputError } from "@openbrains/services";
...
export async function memoryWritebackHandler(
  ...
  try {
    const out = await memoryWriteback(envelope.deps, envelope.auth.userId, rawInput);
    return ok(out);
  } catch (e) {
