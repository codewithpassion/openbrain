import { ServiceAuthError, ServiceInputError, thoughtStats } from "@openbrains/services";
...
export async function thoughtStatsHandler(
  ...
  try {
    const out = await thoughtStats(envelope.deps, envelope.auth.userId, rawInput);
    return ok(out);
  } catch (e) {
